...
If anything, this shows that it is best to pick a lower probability in the earlier 
weeks, as the Opportunity Cost model is First Order Stochastically Dominated by 
both models until Week 7.From there, it appears the Opportunity Cost model FOSD 
the other two models. 

To combat this issue, I will experiment with different discount rates on future 
games. This should place higher weight on picking teams more likely to lose in 
the earlier weeks. We see below that future discounting on the opportunity 
cost discounting is preferred, with $\beta \in \{0.7, 0.8, 0.9\}$ all outperforming 
a $\beta = 1 $ until Week 7.

```{r}
op_cost_9 <- by_oc(past_picks, past_weeks, beta = 0.9)
op_cost_8 <- by_oc(past_picks, past_weeks, beta = 0.8)
op_cost_7 <- by_oc(past_picks, past_weeks, beta = 0.7)

oc9 <- call_sim(op_cost_9, N, "Oppertunity Cost, Beta = 0.9")
oc8 <- call_sim(op_cost_8, N, "Oppertunity Cost, Beta = 0.8")
oc7 <- call_sim(op_cost_7, N, "Oppertunity Cost, Beta = 0.7")

# Compare models
data <- rbind(opp_cost, oc9, oc8, oc7)
