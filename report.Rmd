...
keeping up with the NFL than other competitors in the pool, I chose to make my picks 
optimizing over NFL game outcome forecasts.
Now that the pool is over, I am able to publicize the approach 
I took to maximizing my likelihood of success in the pool. 

For those who haven't heard of a losers pool before, the rules are straight 
...
pot of entry fees. The only catch, once a team is picked, you cannot pick that 
team again. This is where the strategy comes into play, by optimizing when to 
pick each team, one can reduce your overall likelihood of losing from the pool. 
...
week will front-load the best teams and will most likely run into suboptimal picks 
in later weeks. 

The algorithm is straight forward: 

1. Pick the lowest win probability in the start week.
2. Remove the picked team from the candidate pool of teams for the remaining weeks.
3. Move to the next week and repeat. 
...
1. Compute the difference between the lowest and second lowest win probabilities 
per week. 
2. Pick the team with the largest delta, or oppertunity cost across all weeks. 
3. Remove the picked team and week from the pool of options.
4. Repeat step 1.

Compared to the naive approach, this model preforms worse in early weeks, but 
out preforms the naive approach the longer the season goes on. 
...
### Random Search: 
* Work in progress * 
We can find a set of weekly picks that minimizes the total win probability 
using a random search approach. The general principal is to select teams 
randomly conditional on their overall team performance. Then, we take the 
best performing teams per sample, and mutate the teams from their selections 
to derive further results. Ultimately, this should get us towards an optimal 
selection, simply by breeding well performing teams. 

The rough algorithm is the following: 

1. Compute weighted probabilities for each team based on their season average 
win rate. 
2. Sample a set of teams for each week based on the weighting calculated in (1). 
3. Compute the average win probability for each team.
4. ... and so on, with mutations, corss-selection, reproduction etc. 
...
# set up for later
ga_picks = data.frame(week = c(3:10), team = top_chromosome)
ga_picks = left_join(ga_picks, df, by = c("week", "team"))
names(ga_picks) = c("week", "loser", "p_win")
```
...
kbl(tbl, digits=3, caption = "Optimal Picks per Week") %>%
  ...
  add_header_above(c(" " = 1, "Oppertunity Cost" = 2, "Lowest Per Week" = 2, "GA Model" = 2)) %>%
...
models = list(pick[3:10, ], naive[3:10, ], ga_picks)
...
Additionally, we can see the poor performance out of our GA model. This is to be 
expected as it is only using the random breeding and should be taking on a more 
rigerous algo. 
...
ga_picks$approach = "Random Search"
compare = rbind(pick, naive, ga_picks)
